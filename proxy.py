...
init(autoreset=True)
def display_banner():
    banner = pyfiglet.figlet_format("Evil Proxy")
    print(Fore.RED + banner)
    print(Fore.RED + "[>] Welcome to Evil Proxy! [<]")
...
class TcpProxy:
    ...
    def _process_data(self, stream: bytes) -> None:
        ...
        for offset in range(0, len(stream), self._chunk_size):
            ...
            text_view = "".join(
                chr(byte) if 32 <= byte <= 126 else "." for byte in block
            )
    ...
    def orchestrate(self) -> None:
        ...
        # Prompt the user for proxy configuration
        print("[>]Configure the Parameters[<]")
        self._local_addr = input("Enter local address: ").strip()
        self._local_port = int(input("Enter local port: ").strip())
        self._remote_addr = input("Enter remote address: ").strip()
        self._remote_port = int(input("Enter remote port: ").strip())
        self._preload = input("Preload remote response? (yes/no): ").strip().lower() in ["yes"]
        ...
        while True:
            ...
            bridge = threading.Thread(target=self._bridge_connections, args=(client,))
            bridge.daemon = True
            bridge.start()
